...
public class JapStuJFrame extends JFrame
    ...
    private JProgressBar jlptProgressBar;
    ...
    private void initNorthPanel() {
        ...
        jlptProgressPanel = new JPanel(new FlowLayout(FlowLayout.LEFT, 8, 0));
        ...
        jlptProgressBar.setPreferredSize(new Dimension(300, 20));
        jlptProgressBar.setStringPainted(true);
        jlptProgressBar.setFont(new Font("微软雅黑", Font.PLAIN, 12));

        jlptProgressLabel = new JLabel();
        jlptProgressLabel.setFont(new Font("微软雅黑", Font.PLAIN, 13));
        jlptProgressLabel.setForeground(new Color(80, 80, 80));
        jlptProgressLabel.setPreferredSize(new Dimension(500, 24));

        jlptProgressPanel.add(jlptProgressBar);
        jlptProgressPanel.add(Box.createHorizontalStrut(10));
        jlptProgressPanel.add(jlptProgressLabel);
    ...
    private void updateJLPTProgress() {
        ...
        while (p != null) {
            ...
            switch (p.masteryState) {
                case 2: jlptMasteredCount++; break;
                case 1: jlptKnownCount++; break;
                default: jlptStrangeCount++; break;
            }
            p = p.next;
        }
        ...
        int masteredPct = (int)((double)jlptMasteredCount / jlptTotalItems * 100);
        int knownPct = (int)((double)jlptKnownCount / jlptTotalItems * 100);
        jlptProgressBar.setValue(masteredPct + knownPct);

        double masteredPctExact = (double)jlptMasteredCount / jlptTotalItems * 100;
        double knownPctExact = (double)jlptKnownCount / jlptTotalItems * 100;
        double strangePctExact = (double)jlptStrangeCount / jlptTotalItems * 100;

        jlptProgressBar.setString(String.format("%s  %.1f%% / %.1f%% / %.1f%%",
                currentJLPTLevel, strangePctExact, knownPctExact, masteredPctExact));

        jlptProgressLabel.setText(String.format("<html>陌生:%d  了解:%d  掌握:%d  共:%d &nbsp;&nbsp;<font color='#3C78B4'>当前词库: JLPT %s 共%d词</font></html>",
                jlptStrangeCount, jlptKnownCount, jlptMasteredCount, jlptTotalItems, currentJLPTLevel, jlptTotalItems));

        // 自定义进度条颜色
        jlptProgressBar.setUI(new javax.swing.plaf.basic.BasicProgressBarUI() {
            @Override
            ...
            protected void paintDeterminate(Graphics g, JComponent c) {
                Insets b = jlptProgressBar.getInsets();
                int w = jlptProgressBar.getWidth() - b.left - b.right;
                int h = jlptProgressBar.getHeight() - b.top - b.bottom;
                ...
                String txt = jlptProgressBar.getString();
    ...
    private void startJLPTMode() {
        ...
        btnJLPTLevelSwitch.setVisible(true);
        print("===== JLPT " + currentJLPTLevel + " 词书 =====");
        print("共载入 " + jlptTotalItems + " 个词汇");
        print("点击「继续测试」开始测试");
        print("点击「查看全部」浏览全部词汇");
    ...
    private void jlptTestMode() {
        ...
        if (totalNonMastered == 0) {
            print("恭喜！JLPT " + currentJLPTLevel + " 全部词汇已掌握！");
            updateJLPTProgress();
            state = 0;
            return;
        }
        ...
        print("===== JLPT " + currentJLPTLevel + " 测试 =====");
    ...
    private void handleJLPTTestResult(JaNode node, boolean correct) {
        print("\n=====================================");
        print("JLPT " + currentJLPTLevel + " 测试结果");
        print("=====================================");
        print("累计考核：" + node.examTimes + " 次");
        print("正确次数：" + node.trueTimes + " 次");

        String oldState = getMasteryLabel(node.masteryState);
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if (obj == btnImageReco) {
            ...
            if (showAllTargetLocal) {
                ...
                showAllTargetLocal = true;
                if (jlptList == null || listLen(jlptList) == 0) {
                    loadJLPTList();
                }
                showJLPTAsCards();
            }
